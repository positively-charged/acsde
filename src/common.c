...
void str_copy( struct str* str, const char* value, int length ) {
   ...
   memcpy( str->value, value, ( size_t ) length );
   str->value[ length ] = 0;
   str->length = length;
}
...
void str_grow( struct str* string, int length ) {
   string->value = mem_realloc( string->value, ( size_t ) length );
...
void str_append( struct str* str, const char* value ) {
   ...
   memcpy( str->value + str->length, value, ( size_t ) length );
...
void str_append_sub( struct str* str, const char* cstr, int length ) {
   ...
   memcpy( str->value + str->length, cstr, ( size_t ) length );
   str->length += length;
...
void str_append_format( struct str* str, const char* format, va_list* args ) {
   ...
   if ( length >= 0 ) {
      ...
      vsnprintf( str->value + str->length, ( size_t ) ( str->buffer_length - str->length ),
         format, args_copy );
   }
   va_end( args_copy );
}
...
bool c_read_full_path( const char* path, struct str* str ) {
   ...
   if ( realpath( path, str->value ) ) {
      str->length = ( int ) strlen( str->value );
      return true;
   }
   else {
      return false;
